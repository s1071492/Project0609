...
class GamePage2_1 : AppCompatActivity(),View.OnClickListener {
    lateinit var mper: MediaPlayer
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_game_page2_1)
    ...
    override fun onClick(v: View) {
        if(v.id.equals(R.id.btnSad)){
            ...
            {
                intent = Intent(this@GamePage2_1, DrawCheck::class.java)
                intent.putExtra("表情",txvAnswer.text)
                intent.putExtra("頁數",2_1)
